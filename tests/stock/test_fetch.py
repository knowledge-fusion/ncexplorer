def test_fetch_daily_stock_data():
    from app.pipelines.import_news.stock_data_importer import fetch_daily_stock_data

    fetch_daily_stock_data("IEP", "full")
                ...
                UpdateOne(
                    {"symbol": row["Symbol"]},
                    {
                        "$set": {
                            ...
                            "ipo_year": int(row["IPOyear"])
                            if row["IPOyear"].isdigit()
                            else None,
